"""
Documentaries parser for reading markdown files from Obsidian vault.
Parses YAML frontmatter and extracts documentary metadata.

TODO: Update this parser with the actual documentary template structure when available.
"""
...
@dataclass
class Documentary:
    """Represents a documentary parsed from markdown."""

    id: str  # Filename without extension
    title: str
    media_type: str = "documentaries"

    # Documentary-specific fields (based on current implementation)
...
def parse_documentary(file_path: Path) -> Optional[Documentary]:
    """
    Parse a single markdown file and extract documentary metadata.

    Args:
        file_path: Path to the markdown file

    Returns:
        Documentary object or None if parsing fails
    """
    try:
        ...
        print(f"Error parsing documentary {file_path}: {e}")
        return None


def scan_documentaries_directory(directory: Path) -> List[Documentary]:
    """
    Scan directory for all documentary markdown files and parse them.

    Args:
        directory: Path to directory containing markdown files

    Returns:
        List of Documentary objects
    """
    documentaries = []

    if not directory or not directory.exists():
        print(f"Documentaries directory not found: {directory}")
        return documentaries

    # Recursively find all .md files
    for md_file in directory.rglob("*.md"):
        documentary = parse_documentary(md_file)
        if documentary:
            documentaries.append(documentary)

    print(f"Found {len(documentaries)} documentaries in {directory}")
    return documentaries
