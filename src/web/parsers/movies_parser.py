"""
Movies parser for reading markdown files from Obsidian vault.
Parses YAML frontmatter and extracts movie metadata.

TODO: Update this parser with the actual movie template structure when available.
"""
...
@dataclass
class Movie:
    """Represents a movie parsed from markdown."""

    id: str  # Filename without extension
    title: str
    media_type: str = "movies"

    # Movie-specific fields (based on current implementation)
...
def parse_movie(file_path: Path) -> Optional[Movie]:
    """
    Parse a single markdown file and extract movie metadata.
    ...
    try:
        ...
        print(f"Error parsing movie {file_path}: {e}")
        return None


def scan_movies_directory(directory: Path) -> List[Movie]:
    """
    Scan directory for all movie markdown files and parse them.
    ...
    if not directory or not directory.exists():
        print(f"Movies directory not found: {directory}")
        return movies

    # Recursively find all .md files
    for md_file in directory.rglob("*.md"):
        movie = parse_movie(md_file)
        if movie:
            movies.append(movie)

    print(f"Found {len(movies)} movies in {directory}")
    return movies
